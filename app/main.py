...
def apply_mask(img:cv2.typing.MatLike,gray_img:cv2.typing.MatLike, location_countours:array):
...
def save_result(text:str, img:cv2.typing.MatLike,countours:array, file_name:str) -> str:
    font = cv2.FONT_HERSHEY_SIMPLEX
    res = cv2.putText(img, text=text, org=(countours[0][0][0], countours[1][0][1]+60), fontFace=font, fontScale=1, color=(0,255,0), thickness=2, lineType=cv2.LINE_AA)
    res = cv2.rectangle(img, tuple(countours[0][0]), tuple(countours[2][0]), (0,255,0),3)
